...
@HiltViewModel
class SelectReactionViewModel @Inject constructor(val client: TelegramClient, val messageProvider: MessageProvider,) : ViewModel() {
    fun getAvailableReactions(chatId: Long, messageId: Long): Flow<TdApi.AvailableReactions?> {
        return client.sendRequest(TdApi.GetMessageAvailableReactions(chatId, messageId, REACTIONS_PER_ROW)).filterIsInstance()
    }
    ...
    fun addMessageReaction(chatId: Long, messageId: Long, reactionType: TdApi.ReactionType) {
        viewModelScope.launch {
            client.sendUnscopedRequest(TdApi.AddMessageReaction(chatId, messageId, reactionType, false, true))
        }
    }
